...
if (isset($_POST['saveFee'])) {
    ...
    $year = $_POST['year'];
    $fee_details = $_POST['fee_details'];
    if ($branch_id != "" && $year != "" && $fee_details != "") {
        $qryInsert = mysqli_query($conn, "INSERT INTO fee (branch_id, year, fee_details, status) VALUES ('$branch_id', '$year', '$fee_details', '1')") or die(mysqli_error());
...
if (isset($_POST['hid_delete_id'])) {
    ...
    $qryDelete = mysqli_query($conn, "UPDATE fee SET status='0' WHERE fee_id='$delete_id'") or die(mysqli_error());
                        ...
                        <div class="card">
                            ...
                            <div class="card-body">
                                <form method="POST" action="">
                                    ...
                                    <h4 class="card-title">Year: <span class="text-danger">*</span></h4>
                                    <select class='form-control col-12 col-lg-5 input-focus' name="year" id="year" required>
                                    ...
                                    <h4 class="card-title">Fee Details: <span class="text-danger">*</span></h4>
                                    <textarea class='form-control col-12 col-lg-5 input-focus' type="text" name="fee_details" id="fee_details" style="height: 145px;" required></textarea><br>
                                    <div class="col-12 col-lg-5">
                                        <center>
                                            <button class='btn btn-info btn-rounded' type="submit" name="saveFee" id="saveFee" onclick="save()">SAVE</button>
                                        </center>
                                    </div>
                                </form>
                    ...
                    <div class="col-lg-8">
                        <div class="table-responsive">
                            <table class="table table-striped">
                                ...
                                <tbody>
                                    <?php
                                    $getFeeList = mysqli_query($conn, "SELECT * FROM fee WHERE status='1' ORDER BY fee_id DESC") or die(mysqli_error());
                                    $sno = 1;
                                    while ($resFeeList = mysqli_fetch_object($getFeeList)) {
                                        if ($resFeeList->year == '1') {
                                            $DisplayYear = "1st Year";
                                        } else if ($resFeeList->year == '2') {
                                            $DisplayYear = "2nd Year";
                                        } else if ($resFeeList->year == '3') {
                                            $DisplayYear = "3rd Year";
                                        } else if ($resFeeList->year == '4') {
                                            $DisplayYear = "4th Year";
                                        }
                                    ?>
                                        <tr>
                                            <td><?= $sno++ ?></td>
                                            <td><?= getBranchName($conn, 'branch', 'branch_name', 'branch_id', $resFeeList->branch_id) ?></td>
                                            <td><?= $DisplayYear ?></td>
                                            <td><?= $resFeeList->fee_details ?></td>
                                            <td><a class="btn btn-primary" href="javascript:deleteFee('<?= $resFeeList->fee_id ?>')">Delete</a></td>
                                        </tr>
                                    <?php } ?>
                                </tbody>
                            </table>
