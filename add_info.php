...
if (isset($_POST['saveInfo'])) {
    ...
    if ($branch_id != "" && $info_details != "") {
        $qryInsert = mysqli_query($conn, "INSERT INTO information (branch_id, sem_id, info_details, status) VALUES ('$branch_id', '$sem_id', '$info_details', '1')") or die(mysqli_error());
...
if (isset($_POST['hid_delete_id'])) {
    ...
    $qryDelete = mysqli_query($conn, "UPDATE information SET status='0' WHERE info_id='$delete_id'") or die(mysqli_error());
                        ...
                        <div class="card">
                            ...
                            <div class="card-body">
                                <form method="POST" action="">
                                    ...
                                    <select class='form-control col-12 col-lg-5 input-focus' name="sem_id" id="sem_id">
                                    ...
                                    <h4 class="card-title">Information: <span class="text-danger">*</span></h4>
                                    <textarea class='form-control col-12 col-lg-5 input-focus' type="text" name="info_details" id="info_details" style="height: 145px;" required></textarea><br>
                                    <div class="col-12 col-lg-5">
                                        <center>
                                            <button class='btn btn-info btn-rounded' type="submit" name="saveInfo" id="saveInfo" onclick="save()">SAVE</button>
                                        </center>
                                    </div>
                                </form>
                    ...
                    <div class="col-lg-8">
                        <div class="table-responsive">
                            <table class="table table-striped">
                                ...
                                <tbody>
                                    <?php
                                    $getInfoList = mysqli_query($conn, "SELECT * FROM information WHERE status='1' ORDER BY info_id DESC") or die(mysqli_error());
                                    $sno = 1;
                                    while ($resInfoList = mysqli_fetch_object($getInfoList)) {
                                    ?>
                                        <tr>
                                            <td><?= $sno++ ?></td>
                                            <td><?= getBranchName($conn, 'branch', 'branch_name', 'branch_id', $resInfoList->branch_id) ?></td>
                                            <td><?= getBranchName($conn, 'semester', 'sem_name', 'sem_id', $resInfoList->sem_id) ?></td>
                                            <td><?= $resInfoList->info_details ?></td>
                                            <td><a class="btn btn-primary" href="javascript:deleteFaculty('<?= $resInfoList->info_id ?>')">Delete</a></td>
                                        </tr>
                                    <?php } ?>
                                </tbody>
                            </table>
