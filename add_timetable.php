...
if (isset($_POST['saveTT'])) {
    ...
    $tt_details = $_POST['tt_details'];
    if ($branch_id != "" && $sem_id != "" && $exam_id != "" && $tt_details != "") {
        $qryInsert = mysqli_query($conn, "INSERT INTO timetables (branch_id, sem_id, exam_id, tt_details, status) VALUES ('$branch_id', '$sem_id', '$exam_id', '$tt_details', '1')") or die(mysqli_error());
...
if (isset($_POST['hid_delete_id'])) {
    ...
    $qryDelete = mysqli_query($conn, "UPDATE timetables SET status='0' WHERE tt_id='$delete_id'") or die(mysqli_error());
            ...
            <div class="page-breadcrumb">
                <div class="row">
                    <div class="col-5 align-self-center">
                        <h4 class="page-title">Enter Time Table</h4>
                        ...
                        <div class="card">
                            ...
                            <div class="card-body">
                                <form method="POST" action="">
                                    ...
                                    <h4 class="card-title">Exam Type: <span class="text-danger">*</span></h4>
                                    <select class='form-control col-12 col-lg-5 input-focus' name="exam_id" id="exam_id" required>
                                        <option value="">Select Subject</option>
                                        <?php
                                        $getExams = mysqli_query($conn, "SELECT * FROM exam") or die(mysqli_error());
                                        while ($resExams = mysqli_fetch_object($getExams)) {
                                        ?>
                                            <option value="<?= $resExams->exam_id ?>"><?= $resExams->exam_type ?></option>
                                        <?php
                                        }
                                        ?>
                                    </select><br>
                                    <h4 class="card-title">Time Table Details: <span class="text-danger">*</span></h4>
                                    <textarea class='form-control col-12 col-lg-5 input-focus' type="text" name="tt_details" id="tt_details" style="height: 145px;" required></textarea><br>
                                    <div class="col-12 col-lg-5">
                                        <center>
                                            <button class='btn btn-info btn-rounded' type="submit" name="saveTT" id="saveTT" onclick="save()">SAVE</button>
                                        </center>
                                    </div>
                                </form>
                            ...
                            <table class="table table-striped">
                                <thead>
                                    <tr>
                                        ...
                                        <th>Semester</th>
                                        <th>Exam</th>
                                        <th>Time Table Deatils</th>
                                        <th>Action</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    <?php
                                    $getTTList = mysqli_query($conn, "SELECT * FROM timetables WHERE status='1' ORDER BY tt_id DESC") or die(mysqli_error());
                                    $sno = 1;
                                    while ($resTTList = mysqli_fetch_object($getTTList)) {
                                    ?>
                                        <tr>
                                            <td><?= $sno++ ?></td>
                                            <td><?= getBranchName($conn, 'branch', 'branch_name', 'branch_id', $resTTList->branch_id) ?></td>
                                            <td><?= getBranchName($conn, 'semester', 'sem_name', 'sem_id', $resTTList->sem_id) ?></td>
                                            <td><?= getBranchName($conn, 'exam', 'exam_type', 'exam_id', $resTTList->exam_id) ?></td>
                                            <td><?= $resTTList->tt_details ?></td>
                                            <td><a class="btn btn-primary" href="javascript:deleteTT('<?= $resTTList->tt_id ?>')">Delete</a></td>
                                        </tr>
                                    <?php } ?>
                                </tbody>
                            </table>
